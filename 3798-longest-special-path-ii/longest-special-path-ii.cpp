...
public:
    ...
    vector<int>trace;
    void dfs(int node , int par , int s , int e , int pathSum , vector<int>&path , vector<int>&nums,int twice){
        int prev = trace[nums[node]];
        trace[nums[node]] = e;

        while(s <= min(prev,twice)){
             pathSum -= path[s];
             s++;
        }

        if(prev != -1){
            twice = max(prev , twice);
        }
        ...
        for(auto &it: adj[node]){
             if(it.first != par){
                 ...
                 dfs(it.first , node , s , e+1 , pathSum+ it.second ,path , nums , twice);
                 path.pop_back();
             }
        }
